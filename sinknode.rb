...
sub_command = "docker run -it rranshous/mosquitto mosquitto_sub -v -h #{source_host} -p #{source_port} -t 'level/#' -q 2 -i bridge-#{our_level}-#{source_host}-#{target_host}"
puts "opening subprocesses with command: #{sub_command}"

PTY.spawn sub_command do |r, w, pid|
  begin
    puts "syncing"
    r.sync
    puts "reading"
