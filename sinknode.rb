...
level = ARGV.shift
raise 'missing level' if level.nil?
level = level.to_i
...
sub_command = "docker run -it rranshous/mosquitto mosquitto_sub -v -h #{source_host} -t 'level/#' -q 2 -i bridge-#{level}-#{source_host}-#{target_host}"
...
while line = sub_io.readline do
  ...
  # maintain the topic
  pub_command = "docker run -it rranshous/mosquitto mosquitto_pub -h #{target_host} -t #{topic} -s"
